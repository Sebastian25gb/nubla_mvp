...
async def get_anomalies(
    ...
    try:
        ...
        anomalies = []
        ip_counts = defaultdict(int)  # Contador de intentos fallidos por IP
        ip_timestamps = defaultdict(list)  # Tiempos de los intentos por IP para detectar picos
        ip_locations = defaultdict(set)  # Ubicaciones por IP para detectar accesos desde múltiples ubicaciones
        ...
        for log in logs:
            try:
                ...
                if ip and "failed" in log["message"].lower():
                    ...
                    # Detección de picos de actividad: muchas solicitudes en un corto período
